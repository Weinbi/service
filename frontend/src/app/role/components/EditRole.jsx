// frontend/src/app/role/components/EditRole.jsx
...
const EditRole = ({ role, onSuccess, permissionsMap, salarySchemesList, taxSchemesList }) => {
  ...
  useEffect(() => {
    if (role) {
      reset({
        ...
        salary_scheme: Array.isArray(role.salary_scheme) ? role.salary_scheme : JSON.parse(role.salary_scheme || '[]'),
        tax_scheme: Array.isArray(role.tax_scheme) ? role.tax_scheme : JSON.parse(role.tax_scheme || '[]'),
      });
    }
  }, [role, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      await axios.put(`/api/roles/${role.id}`, data);
  ...
  return (
    <div id="role-edit-modal" className="hs-overlay hidden size-full fixed top-0 start-0 z-80 overflow-x-hidden overflow-y-auto pointer-events-none">
      <div className="hs-overlay-animation-target hs-overlay-open:scale-100 hs-overlay-open:opacity-100 scale-95 opacity-0 ease-in-out transition-all duration-200 sm:max-w-2xl sm:w-full m-3 sm:mx-auto min-h-[calc(100%-56px)] flex items-center">
        <div className="w-full flex flex-col bg-white border border-default-200 shadow-sm rounded-xl pointer-events-auto">
          <div className="flex justify-between items-center py-3 px-4 border-b border-default-200">
            <h3 className="font-bold text-default-800">编辑角色</h3>
            <button ref={closeBtnRef} type="button" className="size-8 flex justify-center items-center text-default-500 rounded-full hover:bg-default-100" data-hs-overlay="#role-edit-modal">
              <X className="size-4" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-4 overflow-y-auto">
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input type="text" className="form-input" {...register('role_name', { required: true })} />
                  ...
                  return (
                    <div key={field.id} className="flex gap-2 mb-2 items-center bg-default-50 p-2 rounded">
                      ...
                      <div className="flex w-1/2">
                        <input type="number" className="form-input text-sm rounded-r-none" {...register(`salary_scheme.${index}.value`)} />
                ...
                {taxFields.map((field, index) => {
                  ...
                  return (
                    <div key={field.id} className="flex gap-2 mb-2 items-center bg-default-50 p-2 rounded">
                      ...
                      <div className="flex w-1/2">
                        <input type="number" step="0.01" className="form-input text-sm rounded-r-none" {...register(`tax_scheme.${index}.value`)} />
                        <span className="inline-flex items-center px-3 rounded-r-md border border-l-0 border-default-200 bg-default-100 text-default-500 text-sm">{suffix}</span>
                      </div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button type="button" className="btn btn-soft-secondary" data-hs-overlay="#role-edit-modal">取消</button>
              <button type="submit" className="btn bg-primary text-white">保存修改</button>
            </div>
          </form>
        </div>
      </div>
